...
function initGuesser(){
  ...
  document.getElementById('guessBest').textContent = getStore('best_guesser', getStore('best_guess', '-'));
...
function checkGuess(){
  ...
  if(val === guessState.secret){
    ...
    const best = getStore('best_guesser', getStore('best_guess', 999));
    if(guessState.attempts < best) { setStore('best_guesser', guessState.attempts); document.getElementById('guessBest').textContent = guessState.attempts; }
